 ...
 */
typedef void (^WXXResponseSuccess)(NSURLSessionDataTask * task,id responseObject);

/**
 *  宏定义请求失败的block
 *  @param error 报错信息
 */
typedef void (^WXXResponseFail) (NSURLSessionDataTask *task , NSError *error);
 ...
 *  @param progress 进度
 */
typedef void (^WXXProgress) (NSProgress *progress);

@interface WXXSessionNetWorking : NSObject
