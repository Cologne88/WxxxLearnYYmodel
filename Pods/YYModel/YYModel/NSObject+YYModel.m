...
//foce_inline 取消了函数的参数压栈，减少了调用的开销 且不会产生二义性
...
/// Parse a number value from 'id'.NSCharacterSet配合NSScanner 做了字符串过滤,把id 类型 转换成NSNumber 类型
...
static force_inline NSDate *YYNSDateFromString(__unsafe_unretained NSString *string) {
    ...
    dispatch_once(&onceToken, ^{
        ...
        
        {
            ...
            //数字是格式的长度  等会通过传进来的字符串.length 作为key找对应的date格式；
    ...
    //传进来的字符串.length 作为Block的key找对应的date格式；
...
static force_inline Class YYNSBlockClass() {
    ...
    dispatch_once(&onceToken, ^{
        ...
        //block的父类关系是block -------> NSGlobalBlock ---------> NSBlock
...
@interface _YYModelPropertyMeta : NSObject {
    ...
    BOOL _isKVCCompatible;       ///< YES if it can access with key-value coding 是否能被归档，需要实现 set/get方法.
    BOOL _isStructAvailableForKeyedArchiver; ///< YES if the struct can encoded with keyed archiver/unarchiver 是否支持结构体归档
...
+ (instancetype)metaWithClassInfo:(YYClassInfo *)classInfo propertyInfo:(YYClassPropertyInfo *)propertyInfo generic:(Class)generic {
    ...
    if ((meta->_type & YYEncodingTypeMask) == YYEncodingTypeStruct) {
        ...
        static dispatch_once_t onceToken;
        dispatch_once(&onceToken, ^{//支持归档的结构体类型
...
@interface _YYModelMeta : NSObject {
    ...
    BOOL _hasCustomClassFromDictionary;//自定义映射
}
@end
...
- (instancetype)initWithClass:(Class)cls {
    ...
    // Get white list 在转换过程中只会处理这个数组的属性
    NSSet *whitelist = nil;
...
static id ModelToJSONObjectRecursive(NSObject *model) {
    ...
    [modelMeta->_mapper enumerateKeysAndObjectsUsingBlock:^(NSString *propertyMappedKey, _YYModelPropertyMeta *propertyMeta, BOOL *stop) {
        ...
        if (propertyMeta->_mappedToKeyPath) {
            ...
            // 遍历keypath数组 上边的例子中 [@"user",@"name"]
            for (NSUInteger i = 0, max = propertyMeta->_mappedToKeyPath.count; i < max; i++) {
                
                // 11.3.6.1 取出数组中的key
                NSString *key = propertyMeta->_mappedToKeyPath[i];
                // 11.3.6.2 如果遍历到最后一个的时候，跳出循环
                if (i + 1 == max) { // end
...
/// Generaate a description string 系统的获取对这个模型的描述
